;;;; This file contains the old codes of cl-fast-queues.
;;;;
;;;; For either fifo or lifo queue, an underlay unsafe fifo or lifo structure
;;;; is utilized, and each cell of the structure points to a fixed length safe fifo
;;;; or lifo structure, which length will increase with the increment of the number of items.
;;;;
;;;; When one structure is full, new longer structure will be created and enqueued into the underlay,
;;;; and when one stucture is empty, it should be dequeued. This is how the unsafe versions work.
;;;;
;;;; For safe versions, a global lock is used for each queue object,
;;;; any enqueue or dequeue operations as well as the change of the underlay should acquire this lock first,
;;;; and hence it's not so fast.
;;;;
;;;; The upgraded and currently published version use the safe structures that the underlay's cells point to.
;;;; Acquiring the lock will only when modifying the underlay, that is, only when enqueue or dequeue a cell.
;;;; This speed the operations time up.
