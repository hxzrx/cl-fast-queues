...
(define-test safe-fifo-exp-enqueue-threads :parent safe-fifo-exp
  ...
  #+ccl (ccl:gc)
  (dotimes (i *loop-times*)
    (let ((queue (cl-fast-queues-exp:make-safe-fifo :init-length (+ 2 (random 10))))
        ...
        (let ((it item))
          (push
           (bt:make-thread #'(lambda ()
                               (atomic-incf (atomic-place enqueue-sum) (cl-fast-queues-exp:enqueue it queue))))
           threads)))
      (dolist (th threads)
        (bt:join-thread th))
      (unless (= (atomic-place enqueue-sum) (apply #'+ items))
        (unless (= (atomic-place enqueue-sum) (apply #'+ items))
          (format t "~&A: expect total: ~d, real total: ~d, ~&items: ~d~&queue-to-list: ~d~%"
                  ...
                  items
                  (cl-fast-queues-exp:queue-to-list queue))))
      (is = (length items) (cl-fast-queues-exp:queue-count queue))
      (let ((queue-list (cl-fast-queues-exp:queue-to-list queue)))
        (unless (and (every #'integerp queue-list) (= (apply #'+ items) (apply #'+ queue-list)))
          (format t "~&B: expect total: ~d, real total: ~d, ~&items: ~d~%queue-list: ~d~%raw queue: ~d~%"
                  (apply #'+ items)
                  (apply #'+ (cl-fast-queues-exp:queue-to-list queue))
                  items
                  (cl-fast-queues-exp:queue-to-list queue)
                  queue)))
      (dotimes (j (length items))
        (setf total (+ total (cl-fast-queues-exp:dequeue queue))))
      (is = total (apply #'+ items))
      (true (cl-fast-queues-exp:queue-empty-p queue)))))

(define-test safe-fifo-exp-dequeue-threads :parent safe-fifo-exp
  #+sbcl (sb-ext:gc :full t)
  #+ccl (ccl:gc)
  (dotimes (i *loop-times*)
    (let* ((queue (cl-fast-queues-exp:make-safe-fifo :init-length (+ 2 (random 10)))) ; :waitp t))
           (items (loop for i below (random 100) collect (random 100)))
           (push-threads nil)
           ;;(pop-threads nil)
           (total (apply #'+ items)))
      (dolist (item items)
        (let ((it item))
          (push
           (bt:make-thread #'(lambda ()
                               (cl-fast-queues-exp:enqueue it queue)))
           push-threads)))
          ...
          (apply #'+
                 (loop for j below (length items)
                       collect (bt:join-thread
                                (bt:make-thread #'(lambda () (cl-fast-queues-exp:dequeue queue t)))))))
      (true (cl-fast-queues-exp:queue-empty-p queue)))))
...
(define-test safe-fifo-exp-enqueue-threads2 :parent safe-fifo-exp
  ...
  (dotimes (i *loop-times*)
    ...
    (let* ((queue (cl-fast-queues-exp:make-safe-fifo :init-length (+ 2 (random 10))))
      ...
      (setf real-push-list (cl-fast-queues-exp:queue-to-list queue))
      (unless (= (apply #'+ *send-to-push-list*) (apply #'+ real-push-list))
        (is = (apply #'+ *send-to-push-list*) (apply #'+ (cl-fast-queues-exp:queue-to-list queue)))
        (unless (= (apply #'+ *send-to-push-list*) (apply #'+ (cl-fast-queues-exp:queue-to-list queue)))
          (format t "*send-to-push-list*: ~d~%queue list: ~d~%queue~d~%"
                  *send-to-push-list*
                  (cl-fast-queues-exp:queue-to-list queue) queue)))
...
(define-test safe-fifo-exp-enqueue/dequeue-threads :parent safe-fifo-exp
  ...
  (dotimes (i *loop-times*)
    ...
    (let* ((queue (cl-fast-queues-exp:make-safe-fifo :init-length (+ 2 (random 10))))
      ...
      (dolist (item items)
        (push
         (bt:make-thread #'(lambda () (sb-ext:atomic-push (cl-fast-queues-exp:dequeue queue t) *pop-list*)))
         pop-threads))
...
(define-test safe-lifo-exp-enqueue-threads :parent safe-lifo-exp
  ...
  #+ccl (ccl:gc)
  (dotimes (i *loop-times*)
    (let ((queue (cl-fast-queues-exp:make-safe-lifo :init-length (+ 2 (random 10))))
          (items (loop for i below (random 100) collect (random 100)))
          (threads nil)
          (total 0))
      (dolist (item items)
...
(define-test safe-lifo-exp-dequeue/enqueue-threads :parent safe-lifo-exp
  ...
  (dotimes (i *loop-times*)
    (let* ((num (+ 2 (random 5)))
           (queue (cl-fast-queues-exp:make-safe-lifo :init-length num)) ; :waitp t))
      ...
      (dolist (item items)
        (push
         (bt:make-thread #'(lambda ()
                             (sb-ext:atomic-push (cl-fast-queues-exp:dequeue queue nil)
                                                 *dequeue-list*)))
         pop-threads))
      (dolist (item items)
        (let ((it item))
          (push
           (bt:make-thread #'(lambda ()
                            (sb-ext:atomic-push (cl-fast-queues-exp:enqueue it queue)
                                                *enqueue-list*)))
           push-threads)))
      (dolist (th pop-threads) (bt:join-thread th))
      (dolist (th push-threads) (bt:join-thread th))
