...
(dolist (t-num *threads-num*)
  ...
  (let* ((num-each-thread (truncate (/ *max-times* t-num)))
         ...
         (counter #+sbcl(list 0) #-sbcl(bt2:make-atomic-integer))
         (ts1 (local-time:now))
         (ts2 nil))
    (dotimes (th t-num)
      (bt:make-thread #'(lambda ()
                          (dolist (item lst)
                            (enqueue item queue))
                          #+sbcl(sb-ext:atomic-incf (car counter))
                          #-sbcl(bt2:atomic-integer-incf counter))))
    (dotimes (th t-num)
      (bt:make-thread #'(lambda ()
                          (dolist (item lst)
                            (dequeue queue))
                          #+sbcl(sb-ext:atomic-incf (car counter))
                          #-sbcl(bt2:atomic-integer-incf counter))))
    (loop while (/= #+sbcl(car counter) #-sbcl(bt2:atomic-integer-value counter)
                    (* 2 t-num))
          do (bt:thread-yield))
